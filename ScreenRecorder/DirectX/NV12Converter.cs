		...
		public void Convert(Texture2D input, Texture2D output)
		{
			...
			if(processor == null)
			{
				...
				VideoProcessorContentDescription contentDesc = new VideoProcessorContentDescription()
				{
					...
					InputFrameRate = new Rational(1, 1), InputWidth = inDesc.Width, InputHeight = inDesc.Height,
					OutputFrameRate = new Rational(1, 1), OutputWidth = outDesc.Width, OutputHeight = outDesc.Height,
				...
				{
					Usage = true, RgbRange = false, YCbCrMatrix = true, YCbCrXvYCC = false, NominalRange = (int)VideoProcessorNominalRange.Range_16_235
				});
			...
			{
				Enable = true, OutputIndex = 0, InputFrameOrField = 0, PastFrames = 0, FutureFrames = 0,
				PpPastSurfaces = null, PInputSurface = inputView, PpFutureSurfaces = null
			};
