...
namespace ScreenRecorder.DirectX.Renderer
{
    public sealed class VideoSourcePresenter : IDisposable
    {
        ...
        public VideoSourcePresenter(int width, int height, IntPtr targetHandle, IVideoSource videoSource)
        {
            ...
            presentThread = new Thread(new ParameterizedThreadStart(PresentThreadHandler)) { Name = "VideoSourcePresenter", IsBackground = true };
        ...
        private void PresentThreadHandler(object args)
        {
            ...
            using (SharpDX.Direct3D11.Device device = new SharpDX.Direct3D11.Device(DriverType.Hardware))
            using (var context = device.ImmediateContext)
            using (SwapChain1 swapChain = new SwapChain1(dxgiFactory, device, TargetHandle, ref swapChainDescription, swapChainFullScreenDescription, null))
            ...
            using (var verticesBuffer = new SharpDX.Direct3D11.Buffer(device, (Vector3.SizeInBytes + Vector2.SizeInBytes) * 6, ResourceUsage.Dynamic, BindFlags.VertexBuffer, CpuAccessFlags.Write, ResourceOptionFlags.None, 0))
            using (ColorShader colorShader = new ColorShader())
            {
                ...
                context.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(verticesBuffer, Vector3.SizeInBytes + Vector2.SizeInBytes, 0));
